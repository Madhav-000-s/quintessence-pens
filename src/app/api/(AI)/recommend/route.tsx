...
export async function POST(req: Request) {
    try {
        ...
        const visitedPens: number[] = payload.visited_pens ?? payload.visitedPens ?? []
        ...
        const body = {
            user_id: userId,
            visited_pens: Array.isArray(visitedPens) ? visitedPens : [],
        }

        const res = await fetch(`${getFastAPIBaseUrl()}/recommend`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(body),
            // Ensure server-to-server fetch
            cache: 'no-store',
        })
