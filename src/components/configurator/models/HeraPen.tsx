...
export function HeraPen({
  ...
  // HERA - Slimmer, elegant, graceful design
  const capLength = 3.6;  // Slightly shorter
  const barrelLength = 3.5; // Delicate proportions

  // Radius at different points - SLIMMER for elegance
  const capTopRadius = 0.30;
  const capMaxRadius = 0.38; // Narrower profile
  const capBottomRadius = 0.36;
  const barrelTopRadius = 0.35;
  const barrelMaxRadius = 0.37; // Slim barrel
  const barrelBottomRadius = 0.30;
  const gripTopRadius = 0.29;
  const gripBottomRadius = 0.25;
  const nibLength = 0.85; // Refined nib
