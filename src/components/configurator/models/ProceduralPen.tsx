...
export function ProceduralPen({
  ...
  // Pen dimensions (luxury fountain pen proportions - cigar-shaped)
  const capLength = 3.8;
  const barrelLength = 3.7;

  // Radius at different points for cigar shape (elegant taper)
  const capTopRadius = 0.36;
  const capMaxRadius = 0.44; // Widest point of cap
  const capBottomRadius = 0.41;
  const barrelTopRadius = 0.40;
  const barrelMaxRadius = 0.43; // Widest point of barrel
  const barrelBottomRadius = 0.35;
  const gripTopRadius = 0.34;
  const gripBottomRadius = 0.29;
  const nibLength = 0.95;
